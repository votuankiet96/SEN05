"""Redis access for the OG Live Stream mechanism."""
...
def get_input_client() -> redis.Redis:
    """Return the Redis client used for DP event/state input."""
...
def get_output_client() -> redis.Redis:
    """Return the Redis client used for OG signal output."""
...
def ensure_consumer_group() -> None:
    """Create the Stream mechanism candle event consumer group if it does not already exist."""
    try:
        get_input_client().xgroup_create(
...
def publish_signal(strategy: str, payload: dict[str, Any]) -> str | None:
    """Publish one Stream mechanism signal payload idempotently."""
    signal_id = str(payload.get("signal_id", "") or "")
    if not signal_id:
        logger.warning("stream_signals: missing signal_id, refusing publish")
        return None
    try:
        stream_keys = settings.signal_stream_keys(strategy, payload)
    except ValueError as exc:
        logger.warning("stream_signals: invalid output routing for signal_id=%s: %s", signal_id, exc)
        return None
    if not stream_keys:
        logger.warning("stream_signals: no output streams configured for signal_id=%s", signal_id)
        return None
    ...
    try:
        ...
        logger.warning("stream_signals: publish failed for signal_id=%s: %s", signal_id, exc)
        reset_output_client()
        return None
