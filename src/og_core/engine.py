"""Shared strategy runner for dashboard, export, and future realtime consumers.

This module owns orchestration only: parse date windows, load local OHLCV data,
run the registered strategy pipeline, and return enriched frames. It does not
build chart JSON, CSV files, or notification messages.
"""
...
from og_core.data.loader import load, load_range
...
def run_single_timeframe_strategy(
    ...
    *,
    ...
    enriched = trim_to_window(spec.add_levels(with_signals, params, symbol), date_window)
