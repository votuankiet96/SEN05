"""Typed settings loader for the refactored DP backend.

Source of truth:
- Runtime (operator-editable) values live in `config/dp_provider.env`.
- Instrument/timeframe definitions live in `core_engine/settings/instruments.py`.

This module is code, not configuration data. It reads and validates the config
folder, then exposes small typed groups for the engine.
"""
...
# <root>/src/core_engine/settings/__init__.py, four levels down.
...
BACKEND_LIVE_STDOUT_LOG = BACKEND_CHILD_STDOUT_LOG
BACKEND_HISTORICAL_STDOUT_LOG = BACKEND_CHILD_STDOUT_LOG
...
class EngineSettings:
    ...
    tradingview: TradingViewSettings = TradingViewSettings()
...
SETTINGS = EngineSettings(DB, TRADINGVIEW, HISTORICAL, LIVE, NOTIFICATION, LOGGING, BACKEND)
...
def get_historical_timeframes() -> list[tuple[str, str, str, int]]:
    return [
        (TF_INTERVAL_MAP[tf], tf, TF_STAGING[tf], DEFAULT_N_BARS[tf])
        for tf in ["W", "D1", "H8", "H6", "H4", "H3", "H2", "H1", "M90", "M45", "M30", "M20", "M15", "M10", "M5"]
    ]


def get_live_timeframes() -> list[tuple[str, str, str, int]]:
    return [
        (interval, tf_code, staging_table, LIVE.n_bars)
        for interval, tf_code, staging_table, _ in get_historical_timeframes()
    ]
...
def load_live_settings(logger=None) -> LiveSettings:
    return LIVE


# Compatibility constants for modules while the full engine is being rewired.
SQL_SERVER = DB.server
SQL_DATABASE = DB.database
SQL_DRIVER = DB.driver
SQL_PORT = DB.port
SQL_TDS_VERSION = DB.tds_version
SQL_UID = DB.uid
SQL_PWD = DB.pwd
SQL_ENCRYPT = DB.encrypt
SQL_TRUST_SERVER_CERT = DB.trust_server_cert

TV_AUTH_TOKEN = TRADINGVIEW.auth_token
TV_COOKIE = TRADINGVIEW.cookie
TV_USERNAME = TRADINGVIEW.username
TV_PASSWORD = TRADINGVIEW.password
TV_2FA_SECRET = TRADINGVIEW.two_fa_secret
TV_CAPTCHA_API_KEY = TRADINGVIEW.captcha_api_key
TV_CAPTCHA_SERVICE = TRADINGVIEW.captcha_service
TV_WS_HISTORY_ENDPOINT = TRADINGVIEW.history_endpoint
TV_WS_HISTORY_FALLBACK_ENDPOINTS = TRADINGVIEW.history_fallback_endpoints
TV_WS_HISTORY_REQUEST_MORE_ROUNDS = TRADINGVIEW.history_request_more_rounds
TV_WS_HISTORY_REQUEST_MORE_BARS = TRADINGVIEW.history_request_more_bars
TV_WS_HISTORY_TIMEOUT_SEC = TRADINGVIEW.history_timeout_sec
...
TV_WS_REPLAY_ADVANCE_FACTOR = HISTORICAL.replay_advance_factor
TV_WS_REPLAY_TIMEOUT_SEC = HISTORICAL.replay_timeout_sec

HISTORICAL_PROVIDER = HISTORICAL.provider
STAGING_INSERT_CHUNK_ROWS = HISTORICAL.staging_insert_chunk_rows
N_BARS_LIVE = LIVE.n_bars
DISCORD_WEBHOOK_URL = NOTIFICATION.discord_webhook_url
LOG_LEVEL = LOGGING.level
LOG_FILE = str(PIPELINE_LOG)
