...
def _run_reconcile_fact(args: argparse.Namespace) -> int:
    from core_engine.shared.warehouse.reconcile import reconcile_all, total_missing
...
def _dispatch(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    ...
    if args.command == "live":
        from core_engine.core.live import engine as live_fetching
    ...
    if args.command == "chart-datacheck":
        from core_engine.util.dashboard import server as chart_server
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if argv and argv[0] == "historical":
            from core_engine.core.historical import engine as historical_pulling
