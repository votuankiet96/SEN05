...
Mô tả:
    Kết nối SQL Server qua og_past.data.db_connector, truy vấn bảng
...
Phụ thuộc ngoài:
    og_past.data.db_connector.get_connection() — kết nối pyodbc đến SQL Server.
"""
...
from og_past.data.db_connector import get_connection
from og_core.config import TF_MINUTES, get_symbol
...
def load(symbol: str, tf: str, n_bars: int) -> pd.DataFrame:
    ...
    Giả định giao dịch:
        ...
        Caller cần tự lọc bar đang mở nếu workflow yêu cầu chỉ dùng closed bars.
    """
