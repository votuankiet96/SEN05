...
/*
 * SEN_KNN_Combo_V1 - KNN-filtered Combo CSV signal executor.
 *
 * Workflow:
...
public class SEN_KNN_Combo_V1 : Robot
{
    ...
    [Parameter("Bot Label", Group = "Identity", DefaultValue = "SEN_KNN_Combo_V1")]
    ...
    [Parameter("Max Account Drawdown % (0=off)", Group = "Risk",
        DefaultValue = 30.0, MinValue = 0.0, MaxValue = 100.0, Step = 0.5)]
    ...
    private bool _accountDrawdownTriggered;
    ...
    private int _accountDdTriggerCount;
    ...
    protected override void OnStop()
    {
        ...
        Print("[{0}] Summary | ClustersOpened: {1} | Partial: {2} | Failed: {3} | LegsPlaced: {4} | LegsFailed: {5} | Skip LongBar: {6} | Skip Spread: {7} | Skip BrokerDistance: {8} | Skip Volume: {9} | Skip InvalidRange: {10} | Skip MaxClusters: {11} | Place errors: {12} | SLLadderMoves: {13} | DailyDDHits: {14} | AccountDDHits: {15}",
            BotLabel,
            _clustersOpened,
            _clustersPartial,
            _clustersFailed,
    ...
    private bool ValidateParameters()
    {
        ...
        if (MaxAccountDrawdownPercent < 0 || MaxAccountDrawdownPercent > 100)
        {
            Print("[{0}] ERROR: Max account drawdown percent must be from 0 to 100. Use 0 to disable.", BotLabel);
            return false;
        }
    ...
    private void OpenSignalCluster(SignalInfo signal, DateTime barTime, DateTime nextBarTime, double signalBarClockHours, int signalsInBar)
    {
        ...
        var referenceEntryPrice = useStopOrder ? stopEntryPrice : (isBuy ? Symbol.Ask : Symbol.Bid);
        if (!PassesBrokerDistanceGuard(signal, barTime, referenceEntryPrice, stopLossPips, firstTakeProfitPips))
        {
            _skipBrokerDistanceCount++;
            return;
        }
    ...
    private bool PassesBrokerDistanceGuard(SignalInfo signal, DateTime barTime, double referenceEntryPrice, double stopLossPips, double firstTakeProfitPips)
    {
        var minStopLossPips = GetMinimumDistancePips(Symbol.MinStopLossDistance, referenceEntryPrice);
        var minTakeProfitPips = GetMinimumDistancePips(Symbol.MinTakeProfitDistance, referenceEntryPrice);
    ...
    {
        RestoreDailyDrawdownState();
        RestoreAccountDrawdownState();
    }

    private void RestoreDailyDrawdownState()
    {
        var today = Server.Time.Date;
        var savedDayText = LocalStorage.GetString(GetStorageKey("DailyDay"), LocalStorageScope.Instance);
        var savedEquityText = LocalStorage.GetString(GetStorageKey("DailyStartEquity"), LocalStorageScope.Instance);
        var savedTriggeredText = LocalStorage.GetString(GetStorageKey("DailyTriggered"), LocalStorageScope.Instance);

        if (DateTime.TryParseExact(savedDayText, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var savedDay)
            && savedDay.Date == today
            && TryParseStorageDouble(savedEquityText, out var savedEquity)
            && savedEquity > 0)
        {
            _drawdownDay = today;
            _dayStartEquity = savedEquity;
            _dailyDrawdownTriggered = bool.TryParse(savedTriggeredText, out var savedTriggered) && savedTriggered;

            Print("[{0}] Daily drawdown baseline restored | Day: {1:yyyy-MM-dd} | Start Equity: {2:F2} | Triggered: {3} | Current Equity: {4:F2} | Limit: {5:F1}%",
                BotLabel, _drawdownDay, _dayStartEquity, _dailyDrawdownTriggered, Account.Equity, MaxDailyDrawdownPercent);
            return;
        }
    ...
    private void RestoreAccountDrawdownState()
    {
        var savedPeakText = LocalStorage.GetString(GetStorageKey("AccountEquityPeak"), LocalStorageScope.Instance);
        var savedTriggeredText = LocalStorage.GetString(GetStorageKey("AccountTriggered"), LocalStorageScope.Instance);

        _equityPeak = TryParseStorageDouble(savedPeakText, out var savedPeak) && savedPeak > 0
            ? Math.Max(savedPeak, Account.Equity)
            : Account.Equity;

        _accountDrawdownTriggered = MaxAccountDrawdownPercent > 0
            && bool.TryParse(savedTriggeredText, out var savedTriggered)
            && savedTriggered;

        PersistRiskState(true);

        Print("[{0}] Account drawdown state restored | Equity Peak: {1:F2} | Current Equity: {2:F2} | Triggered: {3} | Limit: {4}",
            BotLabel,
            _equityPeak,
            Account.Equity,
            _accountDrawdownTriggered,
            MaxAccountDrawdownPercent > 0 ? MaxAccountDrawdownPercent.ToString("F1", CultureInfo.InvariantCulture) + "%" : "off");
    }
    ...
    {
        if (CheckAccountDrawdownLimit())
            return true;

        return CheckDailyDrawdownLimit();
    }

    private bool CheckAccountDrawdownLimit()
    {
        if (_equityPeak <= 0)
        {
        ...
        if (!_accountDrawdownTriggered && Account.Equity > _equityPeak)
        {
            _equityPeak = Account.Equity;
            PersistRiskState(false);
        }

        if (MaxAccountDrawdownPercent <= 0 || _equityPeak <= 0)
            return false;

        var drawdownPercent = (_equityPeak - Account.Equity) / _equityPeak * 100.0;
        if (drawdownPercent < MaxAccountDrawdownPercent && !_accountDrawdownTriggered)
            return false;

        if (_accountDrawdownTriggered && !HasBotExposure())
            return true;

        if (!_accountDrawdownTriggered)
        {
            _accountDrawdownTriggered = true;
            _accountDdTriggerCount++;
            PersistRiskState(true);
            Print("[{0}] ACCOUNT DRAWDOWN LIMIT HIT | DD: {1:F2}% | Equity: {2:F2} | Equity Peak: {3:F2} | Balance: {4:F2}. Trading halted until the guard is reset or disabled.",
                BotLabel, drawdownPercent, Account.Equity, _equityPeak, Account.Balance);
        }
    ...
    private bool CheckDailyDrawdownLimit()
    {
        ...
        if (_dailyDrawdownTriggered && !HasBotExposure())
            return true;
    ...
    {
        return GetMyPositions().Length > 0 || GetMyPendingOrders().Length > 0;
    }
    ...
    {
        LocalStorage.SetString(GetStorageKey("DailyDay"), _drawdownDay.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture), LocalStorageScope.Instance);
        LocalStorage.SetString(GetStorageKey("DailyStartEquity"), _dayStartEquity.ToString("R", CultureInfo.InvariantCulture), LocalStorageScope.Instance);
        LocalStorage.SetString(GetStorageKey("DailyTriggered"), _dailyDrawdownTriggered.ToString(CultureInfo.InvariantCulture), LocalStorageScope.Instance);
        LocalStorage.SetString(GetStorageKey("AccountEquityPeak"), _equityPeak.ToString("R", CultureInfo.InvariantCulture), LocalStorageScope.Instance);
        LocalStorage.SetString(GetStorageKey("AccountTriggered"), _accountDrawdownTriggered.ToString(CultureInfo.InvariantCulture), LocalStorageScope.Instance);
