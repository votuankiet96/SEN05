...
def _resolve_strategy_limits(strategy_cfg: dict) -> tuple[float, float]:
    """Resolve daily loss / max DD limits with backward compatibility."""
...
def _max_drawdown_breached(
    ...
    """Return True when account equity violates the configured max-loss rule.

    `fixed_initial` matches FTMO 2-Step Maximum Loss: equity must not fall below
    initial_balance * (1 - max_dd). `trailing_peak` is kept for brokers/rules
    that trail from the account equity peak.
    """
    if max_dd >= 1:
        return False
...
def backtest_fast(symbol: str, df_ind: pd.DataFrame, cfg: dict,
    ...
    # ── Force-close vị thế còn mở khi hết window (đồng bộ với backtest_symbol) ──
    if position and len(bars):
        bar       = bars.iloc[-1]
        d         = position['d']
        ep        = float(bar['close'])
    ...
    if len(rets_fast) > 1 and len(trades_pnl) >= 10 and rets_fast.std() > 0:
        ...
        years_span = max(
            (pd.Timestamp(date_to) - pd.Timestamp(date_from)).days / 365.25, 0.1
        )
