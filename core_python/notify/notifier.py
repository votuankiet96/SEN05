...
class Notifier:
    """Send signal messages through Telegram or Discord without data-provider imports."""
    ...
    def _send_telegram(self, message: str) -> NotifyResult:
        if not self.telegram_token or not self.telegram_chat_id:
            return NotifyResult("telegram", False, "missing TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID")
    ...
    def _send_discord(self, message: str) -> NotifyResult:
        ...
        response = requests.post(self.discord_webhook, json={"content": message[:2000]}, timeout=15)
