...
def _dispatch(args: argparse.Namespace, parser: argparse.ArgumentParser) -> int:
    ...
    if args.command == "live":
        from core_engine import live_fetching
    ...
    if args.command == "chart-datacheck":
        from core_engine.chart_datacheck import server as chart_server
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if argv and argv[0] == "historical":
            from core_engine import historical_pulling
