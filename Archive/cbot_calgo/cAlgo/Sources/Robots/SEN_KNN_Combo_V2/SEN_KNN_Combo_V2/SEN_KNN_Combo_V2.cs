...
/*
 * SEN_KNN_Combo_V2 - FTMO-focused KNN-filtered Combo CSV signal executor.
 *
 * Workflow:
...
public class SEN_KNN_Combo_V2 : Robot
{
    ...
    [Parameter("Bot Label", Group = "Identity", DefaultValue = "SEN_KNN_Combo_V2")]
    ...
    protected override void OnStart()
    {
        ...
        Print("[{0}] Started | Symbol: {1} | TF: {2} | Mode: {3} | EntryStop: {4}% | ExpirationBars: {5} | KSL: {6} ({7:F3}) | TP Profile: {8} -> Levels [{9}] -> Multipliers [{10}] | Risk: {11:F1}%",
            BotLabel,
            SymbolName,
            TimeFrame,
            executionMode,
        ...
        Print("[{0}] FTMO guard | Initial: {1:F2} | Daily rule/buffer/guard: {2:F1}%/{3:F1}%/{4:F1}% | Max loss rule/buffer/guard: {5:F1}%/{6:F1}%/{7:F1}% | Day offset: {8:+#;-#;0}h | Daily floor: {9:F2} | Total floor: {10:F2} | Current equity: {11:F2}",
            BotLabel,
            FtmoInitialCapital,
    ...
    protected override void OnStop()
    {
        ...
        Print("[{0}] Summary | ClustersOpened: {1} | Partial: {2} | Failed: {3} | LegsPlaced: {4} | LegsFailed: {5} | Skip LongBar: {6} | Skip Spread: {7} | Skip BrokerDistance: {8} | Skip Volume: {9} | Skip InvalidRange: {10} | Skip MaxClusters: {11} | Place errors: {12} | SLLadderMoves: {13} | FTMODailyHits: {14} | FTMOMaxHits: {15}",
            BotLabel,
            _clustersOpened,
            _clustersPartial,
            _clustersFailed,
    ...
    private double GetFtmoDailyGuardPercent() => FtmoDailyLossPercent - FtmoDailyLossBufferPercent;

    private double GetFtmoMaximumLossGuardPercent() => FtmoMaximumLossPercent - FtmoMaximumLossBufferPercent;
