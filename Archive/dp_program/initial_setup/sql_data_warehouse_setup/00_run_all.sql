/* ============================================================
   INSTALLER NOTE:
   - This file includes the OHLCV data-provider runtime schema, including
     `05_active_task.sql`.
   - The runtime pulls all 15 timeframes directly from TradingView/Capital.com.
   ============================================================ */
...
/* ============================================================
   ...
   WHAT ARE "TIMEFRAMES"?
     ...
     This database supports 15 timeframes. The runtime pulls all 15
     directly from TradingView/Capital.com — every timeframe (including
     M10, M20, M90, H6, H8) is a native direct pull.
   ...
   ============================================================
   DATABASE ARCHITECTURE — THE TWO-LAYER RUNTIME DESIGN
   ============================================================

   The database is organized into 2 runtime layers (called "schemas"):
   ...
   └────────────────────────┬────────────────────────────────┘
                            │
                   Runtime reads
                   (core_engine queries DWH.Fact_OHLCV directly)
   ...
   Step 1: PULL DATA
     The Python backend engine connects to TradingView
   ...
   Step 4: READ BY RUNTIME AND TOOLS
   ...
   WHAT IT DOES:
     ...
     a) DWH.Dim_Symbol (37 rows, seeded by 06_seed_symbols.sql)
   ...
   ─────────────────────────────
   WHAT IT DOES:
     Creates the runtime ETL procedure:

     a) DWH.usp_LoadDirect (ETL Procedure)
   ...
   FILE 5: 05_active_task.sql
   ─────────────────────────────
   WHAT IT DOES:
     Creates SEN.ActiveTask, the runtime lock/signal table.

   FILE 6: 06_seed_symbols.sql
   ─────────────────────────────
   WHAT IT DOES:
     Seeds DWH.Dim_Symbol with the 37 runtime instruments.

   FILE 7: 07_verify.sql
   ─────────────────────────────
   WHAT IT DOES:
     Runs check queries to confirm everything was created correctly:
       1. Lists all tables (expect 20: 15 staging + ActiveTask + 4 DWH)
       2. Lists DWH procedures (expect DWH.usp_LoadDirect)
   ...
   Q: Why separate SEN and DWH instead of putting everything in one place?
   ...
   ============================================================
     ...
     01_setup_database.sql  — CREATE DATABASE + CREATE SCHEMAs (SEN, DWH)
     02_core_tables.sql     — Dimension tables, Fact_OHLCV
     03_staging_tables.sql  — All 15 SEN.TF_* staging tables
     04_business_objects.sql — ETL stored procedure (DWH.usp_LoadDirect)
     05_active_task.sql     — Runtime lock / token relay table used by Python scripts
     06_seed_symbols.sql    — Seed DWH.Dim_Symbol from the runtime instrument list
     07_verify.sql          — Verification queries; prints SETUP COMPLETE

   External high-frequency provider schemas are not created by this OHLCV setup.
...
:r "05_active_task.sql"
:r "06_seed_symbols.sql"
:r "07_verify.sql"
