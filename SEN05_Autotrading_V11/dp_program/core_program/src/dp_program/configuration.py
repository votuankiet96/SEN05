...
def _validate(config: dict[str, Any]) -> None:
    ...
    redis_cfg.setdefault("backfill_event_channel", "dp:events:backfill")
    ...
    redis_cfg["backfill_event_channel"] = str(
        redis_cfg["backfill_event_channel"] or "dp:events:backfill"
    ).strip()
    ...
    # Live giờ chỉ đẩy thẳng nến lên Redis, không còn ghi SQL -- bật live mà
    # tắt Redis thì live không còn việc gì để làm.
    if live["enabled"] and not redis_cfg["enabled"]:
        raise ConfigError("live.enabled requires redis.enabled=true; live no longer writes SQL")
