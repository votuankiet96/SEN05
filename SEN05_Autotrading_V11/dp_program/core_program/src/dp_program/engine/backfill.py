...
#
# Sau khi ghi SQL xong cho một cặp, backfill tự publish một event nhỏ lên
# Redis (kênh redis.backfill_event_channel) để báo "cặp này vừa có nến mới
# trong SQL" -- cho strategy_lab. Event chỉ mang phạm vi đã đổi (số lượng,
# mốc đầu/cuối), không mang giá trị nến; strategy_lab vẫn tự đọc lại SQL để
# lấy giá trị thật. Đây là client Redis riêng của backfill, độc lập hoàn
# toàn với live.py: fire-and-forget, im lặng khi redis.enabled=false, không
# bao giờ làm hỏng hoặc làm chậm việc ghi SQL (nguồn dữ liệu thật) khi Redis
# trục trặc.
...
def _get_redis_client(settings: dict[str, Any]) -> Any:
...
def _publish_backfill_event(
    config: dict[str, Any], symbol: dict[str, Any], timeframe: dict[str, Any],
    result: dict[str, Any],
) -> None:
    """Best-effort trigger after SQL changes; never raises into the caller."""
    global _redis_client, _redis_circuit_open_until
    settings = config["redis"]
    if not bool(settings["enabled"]) or time.monotonic() < _redis_circuit_open_until:
        return
    payload_dict: dict[str, Any] = {"symbol": symbol["symbol"], "timeframe": timeframe["code"]}
    delivered = result.get("delivered_candles") or []
    if delivered:
        # Phạm vi nến mới/đổi trong lần ghi này. Lấy min/max thật thay vì
        # phần tử đầu/cuối list -- delivered_candles không đảm bảo đã sắp
        # xếp theo thời gian. Định dạng UTC-naive giống live.py:_stamp().
        stamps = [candle["timestamp"].replace(tzinfo=None) for candle in delivered]
        payload_dict["candle_count"] = len(delivered)
        payload_dict["from"] = min(stamps).strftime("%Y-%m-%d %H:%M:%S")
        payload_dict["to"] = max(stamps).strftime("%Y-%m-%d %H:%M:%S")
    payload = json.dumps(payload_dict)
    try:
        _get_redis_client(settings).publish(settings["backfill_event_channel"], payload)
    except redis.RedisError as exc:
        _redis_circuit_open_until = time.monotonic() + int(settings["circuit_cooldown_seconds"])
        _redis_client = None
        log_event(
            LOGGER, logging.WARNING, "REDIS_PUBLISH_FAILED", "MEDIUM", component="redis",
            workflow="backfill", error_type=type(exc).__name__,
            action="event skipped; SQL already committed, no retry",
        )


def shutdown_backfill_redis() -> bool:
    """Close backfill's own Redis connection on service stop."""
    global _redis_client
    closed = True
        ...
        for (symbol, timeframe), plan, request in planned:
            try:
                ...
                if int(result["affected"]) > 0:
                    _publish_backfill_event(config, symbol, timeframe, result)
            except AuthError:
                raise
            except Exception as exc:
