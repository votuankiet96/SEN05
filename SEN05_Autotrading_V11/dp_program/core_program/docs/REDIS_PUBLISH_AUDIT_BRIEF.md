...
> **ĐÃ BỊ THAY THẾ (superseded) kể từ ~2026-09-21.** Hồ sơ audit này mô tả cơ
> chế publish cũ: live ghi SQL trước, một publisher (`util/redis_publisher.py`,
> nay đã xoá) đọc lại từ SQL để mirror sang Redis. Cơ chế đó đã bị thay bằng
> kiến trúc mới: `live` không còn ghi SQL, đẩy thẳng nến từ TradingView lên
> Redis (logic gộp vào `engine/live.py`), tự phát hiện gap và tự phục hồi mỗi
> cycle. Giữ nguyên nội dung bên dưới làm hồ sơ audit lịch sử của thiết kế cũ;
> **không dùng làm căn cứ kỹ thuật cho hệ thống hiện tại.** Muốn audit hệ
> thống hiện tại, đọc trực tiếp `core_program/src/dp_program/engine/live.py`.
