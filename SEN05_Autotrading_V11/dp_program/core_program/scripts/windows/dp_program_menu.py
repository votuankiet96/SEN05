...
def _restart_engine_live() -> None:
    from dp_program_entry import restart_engine

    restart_engine(roles=("live",))
    _pause()


def _restart_engine_backfill() -> None:
    from dp_program_entry import restart_engine

    restart_engine(roles=("backfill",))
    _pause()
...
_ACTIONS: dict[str, Callable[[], None]] = {
    ...
    "3": _stop_engine,
    "4": _restart_engine,
    "5": _restart_engine_live,
    "6": _restart_engine_backfill,
    "7": _view_config_and_status,
    "8": _ops_doctor,
    "9": _view_logs,
    "10": _open_chart,
    "11": lambda: _run_foreground("live"),
    "12": lambda: _run_foreground("backfill"),
    "13": lambda: _run_foreground("live", "backfill"),
    "14": _install_tasks,
    "15": _uninstall_tasks,
}
...
def _print_menu() -> None:
    ...
    print("  4. Restart engine (ap config.yaml moi -- ca live + backfill)")
    print("  5. Restart chi Live (khong dong tram Backfill)")
    print("  6. Restart chi Backfill (khong dong tram Live)")
    print("\n  Chan doan (khong thay doi gi)")
    print("  7. Cau hinh & san sang van hanh (settings + doctor)")
    print("  8. Suc khoe lop van hanh (task, tien trinh, chu ky)")
    print("  9. Xem log gan nhat")
    print(" 10. Mo chart (read-only, http://127.0.0.1:8050)")
    print("\n  Chay tam KHONG qua Task (debug)")
    print(" 11. Chay Live foreground")
    print(" 12. Chay Backfill foreground")
    print(" 13. Chay ca hai foreground")
    print("\n  Cai dat may (can quyen Administrator)")
    print(" 14. Cai/cap nhat Task Scheduler (engine + watchdog)")
    print(" 15. Go toan bo Task")
    print("\n  0. Thoat")
