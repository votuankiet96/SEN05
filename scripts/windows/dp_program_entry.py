...
def watchdog_once(config: dict[str, Any] | None = None) -> int:
    ...
    if restart_needed:
        if _auto_restart_allowed(run_dir):
            print("watchdog: engine treo, dang tu restart...")
            restart_engine(config=config, validate=False)
        else:
            send_watchdog_alert(
                config, "auto_recovery_exhausted",
                {"risk": "CRITICAL", "component": "watchdog",
...
def _engine_alive(config: dict[str, Any]) -> bool:
    return any(_already_running(config, role) for role in _ROLES)
...
def restart_engine(
    config: dict[str, Any] | None = None, *, validate: bool = True, wait_seconds: int = 300,
) -> int:
